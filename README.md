...
- 命令 hook：可执行文件放在共享 git 目录的 `.git/crsu/hooks/`。`pre-diff` / `pre-land` 在写 Crucible 或 rebase/push 之前，非 0 退出则中止。`post-diff` / `post-land` 在成功之后，给外部做收尾（关 session、清 tab）；失败只警告。stdin 是一段 JSON（`version`、`event`、`command`、`review_id`、`url` 等）。`version` 现为 `1`，字段改义或删除时才加一。
